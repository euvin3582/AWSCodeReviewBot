...
def analyze_with_bedrock(diff, previous_findings=None):
    ...
    If `previous_findings` is provided (text from the prior review's
    Findings section), it's appended to the prompt so the model can
    compare the current diff against what it flagged last time and
    explicitly mark resolved findings — covering the case where a fix
    addressed the logic/area of a finding even if it didn't touch the
    exact original line (something Gemini Code Assist misses).
    ...
    try:
        return invoke_model(model, effective_diff)
        ...
        try:
            review = invoke_model(fallback_model, effective_diff)
            return (
...
def find_existing_comment():
    ...
    Returns a tuple (comment_id, comment_body), or (None, None) if there
    isn't one yet (or the lookup fails, in which case we fall back to
        ...
        while api_url:
            ...
            for existing_comment in response.json():
                ...
                if COMMENT_MARKER in body and author == bot_identity_login():
                    return existing_comment["id"], body
...
def extract_previous_findings(comment_body):
    """Extract the Findings section from a previous review comment.

    Returns the raw text of the Findings section (everything between
    '## Findings' and the next '##' heading or the comment footer),
    or None if no Findings section exists (the PR had no issues last time).
    """
    if not comment_body:
        return None
    # Match from "## Findings" to the next "## " heading or the "---" footer
    match = re.search(
        r'## Findings\s*\n(.*?)(?=\n## |\n---)',
        comment_body,
        re.DOTALL
...
def post_review(comment, head_sha, existing_comment_id):
    if comment is None:
...
if __name__ == "__main__":
    ...
    existing_comment_id, existing_comment_body = find_existing_comment()
    previous_findings = extract_previous_findings(existing_comment_body)
    if previous_findings:
        print("Found previous findings from prior review — will ask model to track resolution.")
    review_comments = analyze_with_bedrock(diff, previous_findings)
    post_review(review_comments, head_sha, existing_comment_id)
